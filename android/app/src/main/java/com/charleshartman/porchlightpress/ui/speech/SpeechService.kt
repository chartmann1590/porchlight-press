...
data class SpeechState(val playing: Boolean = false, val title: String = "", val sentence: String = "", val index: Int = -1, val count: Int = 0, val error: String? = null)
...
class SpeechService : Service(), TextToSpeech.OnInitListener {
    ...
    override fun onInit(status: Int) {
        ...
        tts?.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
            ...
            override fun onError(utteranceId: String?) { Log.w("Porchlight", "Speech failed: $utteranceId"); finishSentence() }
    ...
    private fun speak() {
        ...
        if (audio.requestAudioFocus(focus!!) != AudioManager.AUDIOFOCUS_REQUEST_GRANTED) { pause(); return }
        ...
        if (offlineVoice == null) {
            playing = false
            SpeechController.publish(SpeechState(error = "No offline voice is installed for ${language.displayLanguage}. Install one in Android text-to-speech settings."))
    ...
    private fun pause() { playing = false; tts?.stop(); focus?.let(audio::abandonAudioFocusRequest); update() }
    private fun resume() { if (sentences.isEmpty()) return; playing = true; speak() }
    private fun next() {
        if (storyIndex + 1 >= stories.size) { stopPlayback(); return }
        storyIndex++
        title = stories[storyIndex].title
        sentences = splitSentences(stories[storyIndex].text, language)
        index = 0; playing = true; speak()
    }
    ...
    private fun update() {
        SpeechController.publish(SpeechState(playing, title, sentences.getOrNull(index).orEmpty(), index, sentences.size))
        session.setPlaybackState(PlaybackState.Builder()
            ...
            .setState(if (playing) PlaybackState.STATE_PLAYING else PlaybackState.STATE_PAUSED, index.toLong(), speed).build())
        (getSystemService(NOTIFICATION_SERVICE) as NotificationManager).notify(1305, notification())
    }

    private fun stopPlayback() {
        playing = false; tts?.stop(); sentences = emptyList(); stories = emptyList(); index = 0
        SpeechController.publish(SpeechState())
        stopForeground(STOP_FOREGROUND_REMOVE)
        stopSelf()
    }

    private fun pending(actionName: String, request: Int): PendingIntent = PendingIntent.getService(this, request,
        Intent(this, SpeechService::class.java).apply { action = actionName }, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
    ...
    private fun notification(): Notification = Notification.Builder(this, CHANNEL)
        ...
        .addAction(if (playing) android.R.drawable.ic_media_pause else android.R.drawable.ic_media_play,
            if (playing) "Pause" else "Play", pending(if (playing) PAUSE else RESUME, 2))
    ...
    override fun onDestroy() {
        tts?.stop(); tts?.shutdown(); session.release()
